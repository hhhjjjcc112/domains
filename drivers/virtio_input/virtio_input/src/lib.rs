...
use interface::{define_unwind_for_InputDomain, Basic, DeviceBase, InputDomain, VirtioInitInfo};
...
pub struct InputDevDomain {
    input: Once<Mutex<VirtIOInput<HalImpl, VirtioTransport>>>,
}
    ...
    fn init(&self, init_info: &VirtioInitInfo) -> AlienResult<()> {
        let transport = match init_info {
            VirtioInitInfo::Mmio { range, .. } => {
                println!("virtio_input_mmio: {:#x}-{:#x}", range.start, range.end);
            ...
            VirtioInitInfo::Pci {
                ...
                if let (
                    ...
                    *modern_notify_off_multiplier,
                    ...
                    println!(
                        "virtio_input_pci(modern): {:04x}:{:02x}:{:02x}.{}",
                    ...
                    println!(
                        "virtio_input_pci(legacy): {:04x}:{:02x}:{:02x}.{}, io={:#x}-{:#x}",
                    ...
                    VirtioTransport::PciLegacy(
                        LegacyPciTransport::new(Box::new(io_region), DeviceType::Input).unwrap(),
                    )
                } else {
                    panic!("virtio-input pci has no usable transport info");
                }
            }
        };
        let input =
            VirtIOInput::<HalImpl, VirtioTransport>::new(transport).expect("failed to create input driver");
